...
class Agent:
    def __init__(self, width, height, extra_width, gamma):
        self.n_games = 0
        ...
        self.env = Tetris(width, height, extra_width)
        print(self.device)
    ...
    def train(self, num_epochs):
        ...
        while self.n_games < num_epochs:
            ...
            reward, done = self.env.step(action, render=True, n_games=self.n_games)
...
if __name__ == "__main__":
    opt = get_args()
    agent = Agent(width=opt.width, height=opt.height, extra_width=opt.extra_width, gamma=opt.gamma)
